...
\> method:
\>> take zero-bias (BX) data \TODO: which bit
\>> select elastic events (standard tagging) $\rightarrow$ number of events $N({\rm BX,elastic})$
\>> out of the selected events, check how many have trigger flag $\rightarrow$ number of events $N({\rm BX,elastic,trigger})$
\>> then trigger efficiency is obtained as follows:

$$1 - I_{\rm trig} = {N({\rm BX,elastic,trigger}) \over N({\rm BX,elastic})}$$

\> results:
\>> fill 5317, diagonal 45 bot -- 56 top: $N({\rm BX,elastic}) = N({\rm BX,elastic,trigger}) = 7914$
\>> fill 5317, diagonal 45 top -- 56 bot: $N({\rm BX,elastic}) = N({\rm BX,elastic,trigger}) = 7194$

\> \TODO: correction applied or not?


\subsection{DAQ efficiency}

\> method: per time slice calculate from the trigger block of raw data:
$$\hbox{DAQ efficiency} = {\hbox{recorded events}\over \hbox{triggered events}} \equiv 1 - I_{\rm DAQ}$$
...
\>> y: from \plot{resolutions/resolutions_vs_time.pdf}, $\si(m_y) = (0.258 \pm 0.018)\un{\mu rad}$
