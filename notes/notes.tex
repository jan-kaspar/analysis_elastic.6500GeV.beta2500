...
\> effects included
\>> alignment: shifts in $\th^*_{x,y}$, both LR symmetric and anti-symmetric
\>>> in the vertical plane: both correlated and uncorrelated between diagonals
\>> $x-y$ tilts: mixing between $\th^*_{x}$ and $\th^*_{y}$, both LR symmetric and anti-symmetric
\>> optics: preliminary treatment -- scaling of $\th^*_{x,y}$, both LR symmetric and anti-symmetric
...
\>> inefficiency correction (only relevant from 3/4 and 2/4): both slope and intercept
\>> beam momentum
\>> unsmearing: uncertainty of $\si(m_x)$, $\si(m_y)$, fit model dependence
