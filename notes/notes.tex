...
\>>> ``regression'' formula for cuts
\>>> ``hit'' formula for the rest; acceptance calculation assumes that double-arm reconstruction is LR average of single-arm values
\>>> not yet implemented -- only one reconstruction is performed so far

\> formulae currently used
\>> horizontal plane
...
\> method: applied per unit, applied per time slice, illustrated in \plot{alignment/alignment_method.pdf}
...
\> current n-tuples contain alignment $\rightarrow$ expect corrections compatible with 0

\> \plot{alignment/alignment_details_DS-fill5313.pdf}: detailed example of the method
...
\> \plot{angular_diff_vs_time.pdf}: differences (left-right, 210m-220m) in reconstructed scattering angles

\> \plot{mean_th_x_vs_th_y.pdf}: mean of $\th_x^*$ as a function of $\th_y^*$ -- test for residual mis-rotations

\> \plot{mean_th_x_vs_time.pdf}:  mean of $\th_x^*$ as a function of time -- test for stability of $x$ alignment
