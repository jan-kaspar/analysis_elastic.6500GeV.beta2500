...
for (int rpi : rps.keys)
{
	...
	for (int dgi : diagonals.keys)
	{
		...
		for (int dsi : datasets.keys)
		{
			...
			string f = topDir4RP + datasets[dsi] + "/eff3outof4_fit.root";
		...
		// get global fits	
		string f = topDir4RP + "efficiency_fits/global_fit.root";
		string d = diagonals[dgi] + "/" + rps[rpi];
		...
		SetPad(p_a);
		draw((-0.5, a_g)--(datasets.length-0.5, a_g), heavygreen + dashed + 2pt);

		SetPad(p_b);
		draw((-0.5, b_g)--(datasets.length-0.5, b_g), heavygreen + dashed + 2pt);
