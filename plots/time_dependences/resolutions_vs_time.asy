...
{
	NewPad("time $\ung{h}$", "mean~of $\De^{R-L}\th_x^*\ung{\mu rad}$");
	...
	for (int dgni : diagonals.keys)
	{
		draw(swToHours*scale(1, 1e6),
			RootGetObject(topDir+datasets[dsi]+"/distributions_"+diagonals[dgni]+".root", "time dependences/p_diffLR_th_x_vs_time"), "eb,d0",
...
for (int dsi : datasets.keys)
{
	NewPad("time $\ung{h}$", "mean~of $x^*\ung{\mu m}$");
	...
	for (int dgni : diagonals.keys)
	{
		draw(swToHours*scale(1, 1e3),
			RootGetObject(topDir+datasets[dsi]+"/distributions_"+diagonals[dgni]+".root", "time dependences/p_vtx_x_vs_time"), "eb,d0",
			dgn_pens[dgni], mCi+1pt+dgn_pens[dgni], dgn_labels[dgni]);
	}
...
for (int dsi : datasets.keys)
{
	...
	for (int dgni : diagonals.keys)
	{
		...
		DrawBeamDivergence(obj_vtx_x, dgn_pens[dgni], mCi+1pt+dgn_pens[dgni], dgn_labels[dgni]);
	}
...
for (int dsi : datasets.keys)
{
	...
	for (int dgni : diagonals.keys)
	{
		...
		DrawMeanSensorResolution(obj_vtx_x, obj_diffRL_vtx_x, dgn_pens[dgni], mCi+1pt+dgn_pens[dgni], dgn_labels[dgni]);
	}

	//xaxis(YEquals(11.38, false), blue);
	//xaxis(YEquals(11.58, false), red);
...
for (int dsi : datasets.keys)
{
	NewPad("time $\ung{h}$", "mean~of $\De^{R-L}\th_y^*\ung{\mu rad}$");
	currentpad.yTicks = RightTicks(0.2, 0.1);
	...
	{
		draw(swToHours*scale(1, 1e6),
			RootGetObject(topDir+datasets[dsi]+"/distributions_"+diagonals[dgni]+".root", "time dependences/p_diffLR_th_y_vs_time"), "eb,d0",
			dgn_pens[dgni], mCi+1pt+dgn_pens[dgni], dgn_labels[dgni]);
	}
